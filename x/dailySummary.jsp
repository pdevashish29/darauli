...
<%-- <link rel="stylesheet" href="<%=path%>/resources/css/kendo.common.min.css" style="text/css">
<link rel="stylesheet" href="<%=path%>/resources/css/kendo.default.min.css" style="text/css"> --%>
...
app.controller('dailySumaryCtrl', function($scope, dailySumaryFac, $compile){
	$scope.fromDate='';
	$scope.toDate='';
	$scope.month='<%=month%>';
	$scope.year='<%=year%>';
	...
	dailySumaryFac.getDailySummary($scope.fromDate, $scope.toDate, $scope.month, $scope.year).then(
		function(data){
			...
			$scope.inventoryTotal=Object.keys($scope.inventory).length;
			//alert($scope.inventoryTotal);
		}		
	);
	...
	$scope.viewbyDate=function(){
		...
		dailySumaryFac.getDailySummary($scope.fromDate, $scope.toDate, $scope.month, $scope.year).then(
				function(data){
					...
					$scope.inventoryTotal=Object.keys($scope.inventory).length;
					//alert(JSON.stringify($scope.inventory));
	...
	<div class="form-group" style="margin-top: -9px;margin-left: 2px;">
			<div class="panel panel-primary" style="min-height: 500px;">
				<div class="panel-heading">
					<div class="panel-title">Daily Close Summary</div>
				</div>
				<div class="panel-body">
					<div class="form-group">
						<div class="input-group  date" id="fromDateSpan" style="margin-right: 8px; width: 20%;float: left;" >
							...
							<div class="panel-body" id="table_revenue_hide">
								<table class="table table-bordered" >
									<tbody>
											...
											<tr><td style="text-align: left;">Total Estimates including Invoices</td><td>{{revenue[6] |number}}</td><td>{{100|number:2}}%</td></tr>
											<tr><td style="text-align: left;border-bottom :1px solid #000;">Total Invoices</td><td style="border-bottom :1px solid #000;"><span ng-if="revenue[7]>0">{{revenue[7] |number}}</span><span ng-if="revenue[7]==null || revenue[7]==0">{{0|currency}}</span></td><td style="border-bottom :1px solid #000;"><span ng-if="revenue[6]>0">{{revenue[7]*100/revenue[6]| number:2}}%</span><span  ng-if="revenue[6]==0">{{0| number:2}}</span>%</td></tr>
						...
						<div class="panel panel-info" id="div_cash_collection" style="margin-left: -15px;margin-right: -15px;">
							...
							<div class="panel-body" id="table_cash_hide">
								<table class="table table-bordered">
									...
									<tfoot>
										<tr >
											<th>Total</th><th><span ng-if="financialTotal>0">{{financialTotal|currency}}</span><span ng-if="financialTotal==0 || financialTotal==null">{{0|currency}}</span></th><th>{{100|number:2}}%</th>
										</tr>
									</tfoot>
								</table>
							</div>
						...
						<div class="panel panel-info" id="div_inventory_consume_sales" style="margin-left: -15px;margin-right: -15px;">
							...
							<div class="panel-body" id="table_inventory_hide">
								<table class="table table-bordered">
									...
									<tbody ng-init="inv.total={}">
											<tr data-ng-repeat="item in inventory track by $index" >
												<td style="text-align: left;">{{item[0]}}</td>
												<td style="text-align: center;">{{item[1]}}</td>
												<td>{{item[4]|currency}}</td>
												<td ng-init="inv.total.totalQty =inv.total.totalQty+item[2]" xyx="{{totalQty}}">{{item[2]|number:2}}</td>
												<td ng-init="inv.total.totalInventory =inv.total.totalInventory+(item[4]*item[2])">{{item[4]*item[2]|currency}}</td>
												<td>{{item[3]|number:2}}</td> 
											</tr>
											 <tr ng-if="inventoryTotal==0">
												<td colspan="6" style="color: red;font-size: 16px;font-weight: bold;text-align: center;">No record found</td>
											</tr>
											
									</tbody>
									<tfoot ng-if="inventoryTotal>0">
										<tr>
											<th colspan="3">Total Inventory Consumed</th>
											<th >{{inv.total.totalQty|number:2}}</th>
											<th>{{inv.total.totalInventory|currency}}</th>
											<th></th>
										</tr>
									</tfoot>
								</table>
								...
								<table class="table table-bordered">
									...
									<tbody xyx="{{totalPurchaseAmount}}">
											...
											<tr ng-if="purchase.length==0">
												<td colspan="3" style="color: red;font-size: 16px;font-weight: bold;text-align: center;">No record found</td>
											</tr>
									</tbody>
									<tfoot>
										<tr style="font-weight: bold;" ng-if="purchase.length!=0">
											<td >Total Parts Purchases </td>
											<td >{{totalPurchaseAmount|currency}}</td>
											<td >100%</td>
										</tr>
									</tfoot>
								</table>
